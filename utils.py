...
class VisionTransformer(nn.Module):
    def __init__(self, model):
        ...
        # encoder
#         self.dropout = copy.deepcopy(model.encoder.dropout)
#         self.layers1 = copy.deepcopy(model.encoder.layers[0])
#         self.layers2 = copy.deepcopy(model.encoder.layers[1])
#         self.layers3 = copy.deepcopy(model.encoder.layers[2])
#         self.layers4 = copy.deepcopy(model.encoder.layers[3])
#         self.layers5 = copy.deepcopy(model.encoder.layers[4])
#         self.layers6 = copy.deepcopy(model.encoder.layers[5])
#         self.layers7 = copy.deepcopy(model.encoder.layers[6])
#         self.layers8 = copy.deepcopy(model.encoder.layers[7])
#         self.layers9 = copy.deepcopy(model.encoder.layers[8])
#         self.layers10 = copy.deepcopy(model.encoder.layers[9])
#         self.layers11 = copy.deepcopy(model.encoder.layers[10])
#         self.layers12 = copy.deepcopy(model.encoder.layers[11])
...
#                 out = self.encoder.layers[i].ln_1(x)
#                 out, _ = self.encoder.layers[i].self_attention(query=out, key=out, value=out, need_weights=False)
#                 out = self.encoder.layers[i].dropout(out)
#                 out = out + x
                
#                 y = self.encoder.layers[i].ln_2(out)
#                 y = self.encoder.layers[i].mlp(y)
...
def visualize(img, attention, idx):
    ...
    attention = nnf.interpolate(torch.mean(attention[idx], 0).unsqueeze(0).unsqueeze(0), size = size).squeeze().cpu().detach().numpy()
    plt.imshow(attention)
    plt.imshow(img[idx].cpu().permute(1,2,0), alpha = 0.3)
    plt.show()
